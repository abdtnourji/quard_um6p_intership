...
\begin{columns}[T,onlytextwidth]
    ...
    \end{column}
\end{columns}
\begin{alertblock}{Professional Perspective}
The objective is not only to make a demonstration run once. The objective is to build a system that another engineer can understand, reproduce, verify, and improve.
\end{alertblock}
\end{frame}
...
\end{alertblock}
\begin{exampleblock}{Central Principle}
Do not memorize isolated commands. Understand each component's responsibility, its inputs and outputs, its connections, and the observation that proves it works.
\end{exampleblock}
\end{frame}
...
\end{enumerate}
\begin{alertblock}{Day 1 Perspective}
You are not expected to master every technology immediately. First build a reliable mental map; then develop each branch through focused tutorials and practical work.
\end{alertblock}
\end{frame}
...
These foundations let students install the environment, obtain the repository, run documented commands, diagnose problems, preserve changes, and share verified results.
\end{exampleblock}
\end{frame}
