...
\framesubtitle{Move from discovery to quantitative verification}
...
\(\longrightarrow\)
&
Discover channels and message types
\\
\texttt{topic info --verbose}
&
\(\longrightarrow\)
&
Inspect connected endpoints and QoS
\\
\texttt{topic type}
&
\(\longrightarrow\)
...
\\
\texttt{interface show}
&
\(\longrightarrow\)
&
Inspect the message fields
\\
\texttt{topic echo}
&
\(\longrightarrow\)
&
Observe live message values
\\
\texttt{topic hz}
&
\(\longrightarrow\)
&
Measure the message frequency
\end{tabular}
\end{center}

\pause

\begin{exampleblock}{Engineering Question Sequence}
\begin{enumerate}
    \setlength{\itemsep}{0.14em}
    \item Does the topic exist?
    \item Which nodes are connected?
    \item What message structure is expected?
    \item Are messages actually arriving?
    \item Do the values make sense?
    \item Is the publication frequency appropriate?
\end{enumerate}
\end{exampleblock}

\begin{alertblock}{Project Connection}
The same workflow will later be used to inspect PX4 odometry,
trajectory setpoints, camera images, and object-detection results.
\end{alertblock}
